...
use kampela_system::{
    devices::{
        ...
        se_aes_gcm::{decode_bytes, encode_bytes, Protected, ENCODED_LEN},
        se_rng,
...
impl UI {
    ...
    pub fn handle_derivation(&mut self, path: Vec<u8>) {
        self.state.platform.set_derivation(path);
        self.update_request = self.state.handle_derivation(&mut ());
    }
}
...
impl Platform for Hardware {
    ...
    fn set_derivation(&mut self, path: Vec<u8>) {
        self.derivation = String::from_utf8(path).unwrap();
    }
    ...
    fn read_derivation(&mut self) {
        let mut protected = Protected([0; ENCODED_LEN]);
        if let Err(_) = read_data(0x100, &mut protected.0) {
            panic!("Failed to read derivation path");
        };
        if protected.0[0] != 0 && protected.0[0] != 255 {
            self.derivation = String::from_utf8(decode_bytes(&protected)).expect("Failed to decode derivation path");
        }
    }

    fn store_derivation(&mut self) {
        let protected = encode_bytes(&self.derivation.as_bytes());
        if let Err(_) = store_data(0x100, &protected.0) {
            panic!("Failed to save derivation path");
        };
    }
