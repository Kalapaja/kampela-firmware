...
use crate::{derivation::Derivation, dialog::Dialog, display_def::*, pin::pin::Pincode, qr, transaction::{Transaction, TransactionPage}, widget::view::ViewScreen};
...
impl<P: Platform> Screen<P> {
    pub fn get_unit(&self) -> Option<UnitScreen> {
        match self {
            ...
            Screen::ShowDerivation(_) => Some(UnitScreen::ShowDerivation),
...
impl <P: Platform, D: DrawTarget<Color = BinaryColor>> UIState<P, D> {
    pub fn new(mut platform: P, display: D, h: &mut <P as Platform>::HAL) -> Self
        ...
        platform.read_derivation();
        let initial_screen: Option<UnitScreen>;
        let unlocked: bool;
        if platform.public(h).is_none() {
    ...
    fn switch_screen(&mut self, s: Option<UnitScreen>, h: &mut <P as Platform>::HAL )
        ...
        if let Some(s) = s {
            match s {
                ...
                UnitScreen::ShowDerivation => {
                    self.screen = Screen::ShowDerivation(Derivation::new())
                }
            }
        }
    }
    ...
    where <P as Platform>::AsWordList: Sized
    {
        ...
        match self.screen {
            ...
            Screen::ShowDerivation(ref mut a) => {
                let (res, store) = a.handle_tap_screen(point, ());
                if store {
    ...
    pub fn handle_derivation(&mut self, h: &mut <P as Platform>::HAL) -> Option<UpdateRequest> {
        let screen = Some(UnitScreen::ShowDerivation);
        self.switch_screen(screen, h);
        Some(UpdateRequest::UltraFast)
    }
    ...
    where <P as Platform>::AsWordList: Sized
    {
        ...
        match self.screen {
            ...
            Screen::ShowDerivation(ref mut a) => {
                let (res, _) = a.draw_screen(display, self.platform.derivation())?;
                out = res.request;
                new_screen = res.state;
            },
            Screen::QRSignature => {
            ...
            Screen::QRAddress => {
                let line1 = format!("substrate:0x{}", hex::encode(self.platform.public(h).expect("no entropy stored, no address could be shown").0));
