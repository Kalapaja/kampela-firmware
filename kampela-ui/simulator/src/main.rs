...
struct DesktopSimulator {
    ...
    derivation: String,
    stored_derivation: Option<String>,
...
impl Platform for DesktopSimulator {
    ...
    fn store_derivation(&mut self) {
        self.stored_derivation = Some(self.derivation.clone())
    }

    fn read_derivation(&mut self) {
        self.derivation = self.stored_derivation.clone().unwrap_or_default();
        println!("derivation read from emulated storage: {:?}", &self.derivation);
    }

    fn set_derivation(&mut self, path: Vec<u8>) {
        self.derivation = String::from_utf8(path).expect("derivation path is valid string");
    }
