    ...
    fn advance(&mut self, _: ()) -> bool {
        match self.state{
            ...
            EPDInitState::WakeUp(ref mut a) => {
                if display_is_busy() != Ok(false) { return false };
                if a.advance(()) {
                    self.change(EPDInitState::SetTemp(SetTemp::new()))
                }
                false
            },
            EPDInitState::SetTemp(ref mut a) => {
                a.advance(())
            }
        }
    }
...
impl Operation for SetTemp {
    type Input<'a> = ();
    type Output = bool;
    type StateEnum = SetTempState;

    fn new() -> Self {
        Self {
            state: SetTempState::C1(EPDCommand::<0x18>::new()),
            timer: 0,
        }
    }

    fn wind(&mut self, state: SetTempState, delay: usize) {
        self.state = state;
        self.timer = delay;
    }
    ...
    fn advance(&mut self, _: ()) -> bool {
        match self.state{
            SetTempState::C1(ref mut a) => {
                if a.advance(()) {
                    self.change(SetTempState::D1(EPDDataB::<0x80>::new()));
                }
                false
            },
            SetTempState::D1(ref mut a) => {
                if a.advance(()) {
                    self.change(SetTempState::C2(EPDCommand::<0x22>::new()));
                }
                false
            },
            SetTempState::C2(ref mut a) => {
                if a.advance(()) {
                    self.change(SetTempState::D2(EPDDataB::<0xB1>::new()));
                }
                false
            },
            SetTempState::D2(ref mut a) => {
                if a.advance(()) {
                    self.change(SetTempState::C3(EPDCommand::<0x20>::new()));
                }
                false
            },
            SetTempState::C3(ref mut a) => {
                if a.advance(()) {
                    self.change(SetTempState::C4(EPDCommand::<0x1A>::new()));
                }
                false
            },
            SetTempState::C4(ref mut a) => {
                if display_is_busy() != Ok(false) { return false };
                if a.advance(()) {
                    self.change(SetTempState::D41(EPDDataB::<0x64>::new()));
                }
                false
            },
            SetTempState::D41(ref mut a) => {
                if a.advance(()) {
                    self.change(SetTempState::D42(EPDDataB::<0x00>::new()));
                }
                false
            },
            SetTempState::D42(ref mut a) => {
                if a.advance(()) {
                    self.change(SetTempState::C5(EPDCommand::<0x22>::new()));
                }
                false
            },
            SetTempState::C5(ref mut a) => {
                if a.advance(()) {
                    self.change(SetTempState::D5(EPDDataB::<0x91>::new()));
                }
                false
            },
            SetTempState::D5(ref mut a) => {
                if a.advance(()) {
                    self.change(SetTempState::C6(EPDCommand::<0x20>::new()));
                }
                false
            },
            SetTempState::C6(ref mut a) => {
                a.advance(())
            },
        }
    }
    ...
    fn advance(&mut self, _: ()) -> bool {
        ...
        match self.state {
            UpdateFastState::UpdateC1(ref mut a) => {
                if a.advance(()) {
                    self.change(UpdateFastState::UpdateD1(EPDDataB::<0xC7>::new()));
                }
                false
            },
